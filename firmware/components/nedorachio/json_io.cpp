...
bool parse_config_json(const std::string &json, OperationalConfig &out) {
  ...
  out.global.maximum_runtime_minutes = g["maximum_runtime_minutes"] | 60.0f;
  out.global.no_flow_grace_s = g["no_flow_grace_seconds"] | 60.0f;
  out.global.no_flow_sustain_s = g["no_flow_sustain_seconds"] | 30.0f;
  out.global.rain_mm_threshold_48h = g["rain_accumulation_threshold_mm_48h"] | 5.0f;
  out.global.rain_hold_hours_after_forecast = g["rain_accumulation_hold_hours_after_threshold"] | 24.0f;
  ...
  for (JsonPair kv : zones) {
    ...
    const char *mode = z["mode"] | "gallons_target";
    zc.schedule_mode = (strcmp(mode, "gallons_target") == 0) ? 1 : 0;
    zc.goal_gallons = z["goal_gallons_per_cycle"] | 0.0f;
    zc.cycle_gallons = z["cycle_gallons"] | 0.0f;
    zc.soak_minutes = z["soak_minutes"] | 0.0f;
    zc.min_interval_hours = z["minimum_interval_hours"] | 72.0f;
...
bool parse_runtime_json(const std::string &json, RuntimeState &out) {
  ...
  if (deserializeJson(doc, json) != DeserializationError::Ok)
    return false;
  out.version = doc["version"] | doc["v"] | 1;
  ...
  out.last_non_completed_attempt_epoch = doc["last_non_completed_attempt_epoch"] | doc["a"] | 0;
  ...
  for (JsonPair kv : zones) {
    ...
    out.zones[zid - 1].cycle_delivered_gallons = z["cycle_delivered_gallons"] | z["dg"] | 0.0f;
  }
  return true;
}
...
std::string serialize_runtime_json(const RuntimeState &state) {
  ...
  doc["a"] = state.last_non_completed_attempt_epoch;
  ...
  for (int zid = 1; zid <= kNumZones; zid++) {
    ...
    z["dg"] = state.zones[zid - 1].cycle_delivered_gallons;
  }
  std::string out;
