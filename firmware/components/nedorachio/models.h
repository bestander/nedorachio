...
static constexpr uint32_t kHaWeeklyStalenessSeconds = 900u;
...
struct ZoneConfig {
  bool enabled{false};
  float weekly_goal_gallons{0.0f};
...
struct GlobalConfig {
  ...
  float max_attempt_minutes{30.0f};
  float no_flow_grace_s{60.0f};
  float no_flow_sustain_s{30.0f};
  float rain_credit_mm_per_step{10.0f};
  float rain_credit_gallons_per_zone_per_step{100.0f};
  float rain_sensor_hold_hours_after_wet{24.0f};
...
struct OperationalConfig {
  ...
  float rain_mm_this_week{0.0f};
  uint32_t rain_mm_last_pushed_epoch{0};
  uint32_t fallback_start_epoch{kFallbackStartEpoch};
  int last_served_zone_id{0};
};

struct ZoneRuntime {
  uint32_t last_finished_epoch{0};
  uint32_t scheduled_next_epoch{0};
  float weekly_delivered_shadow{0.0f};
  float ha_weekly_delivered{0.0f};
  uint32_t last_attempt_epoch{0};
